...
export async function GET(req: Request, { params }: { params: { matricule: string } }) {
  ...
  try {
    const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM donation WHERE matricule = ?', [matriculeInt]);
    if (rows.length > 0) {
      return NextResponse.json(rows[0]);
    } else {
      return NextResponse.json({ message: 'Donation non trouvée' }, { status: 404 });
    }
  } catch (error) {
    console.error('Erreur lors de la récupération des données:', error);
    return NextResponse.json({ error: 'Erreur lors de la récupération de la donation' }, { status: 500 });
  }
}
...
export async function PUT(req: Request, { params }: { params: { matricule: string } }) {
  ...
  try {
    const [result] = await pool.query<OkPacket>(
      'UPDATE donation SET type = ?, montant = ?, date = ?, groupe_matricule = ?, personne_matricule = ? WHERE matricule = ?',
      [type, montant, date, groupeMatricule, personneMatricule, matriculeInt]
    );
    if (result.affectedRows > 0) {
      return NextResponse.json({ message: 'Donation mise à jour avec succès' });
    } else {
      return NextResponse.json({ message: 'Donation non trouvée' }, { status: 404 });
    }
  } catch (error) {
    console.error('Erreur lors de la mise à jour des données:', error);
    return NextResponse.json({ error: 'Erreur lors de la mise à jour de la donation' }, { status: 500 });
  }
}
...
export async function DELETE(req: Request, { params }: { params: { matricule: string } }) {
  ...
  try {
    const [result] = await pool.query<OkPacket>('DELETE FROM donation WHERE matricule = ?', [matriculeInt]);
    if (result.affectedRows > 0) {
    ...
    return NextResponse.json({ error: 'Erreur lors de la suppression de la donation' }, { status: 500 });
  }
}
